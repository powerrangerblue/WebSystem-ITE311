...
<div class="container-fluid p-4">
    ...
    <div class="mb-4">
        ...
        <p class="text-muted mb-0">Review and approve/decline all student enrollment requests across all courses.</p>
    </div>

    <?php if (empty($pendingRequests)): ?>
        <div class="alert alert-info">
            <h5>No Pending Requests</h5>
            <p>There are currently no pending enrollment requests in the system.</p>
        </div>
    <?php else: ?>
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <small class="text-muted">Courses Affected</small>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    ...
                    <small class="text-muted">Students Waiting</small>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    <?php
                    $uniqueTeachers = array_unique(array_column($pendingRequests, 'teacher_id'));
                    ?>
                    <h4 class="text-warning mb-0"><?= count($uniqueTeachers) ?></h4>
                    <small class="text-muted">Teachers Involved</small>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white">
            <h5 class="mb-0">All Pending Enrollment Requests (<?= count($pendingRequests) ?>)</h5>
        </div>
                            ...
                            <tr data-request-id="<?= esc($request['id']) ?>">
                                ...
                                <td>
                                    <span class="text-muted">
                                        <?= esc($request['teacher_name'] ?? 'Unassigned') ?>
                                    </span>
                                </td>
                                <td>
                                    <span class="text-muted">
...
function processEnrollmentRequest(formData, action) {
    $.ajax({
        url: '<?= base_url('/admin/enrollment-requests/process') ?>',
        type: 'POST',
        data: formData,
        dataType: 'json',
